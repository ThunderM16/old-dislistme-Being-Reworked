...
const dbl = new disbots("ODkxNDQ5NjMzNDA5MjA0Mjc0.YU-hKg.3gH5vPJhe5yQ6v_wJGJRzSEAso0", client);
/*=======================================================================================*/
...
if(!uptimerate)
        ...
        if(breh === newPresence.userID)
      {
       ...
       client.channels.cache.get("893198095528103967").send(`<@${newPresence.userID}> is Offline And Uptime Rate - ${uptimerate}%`) 
   ...
   if(!uptimerate)
      {
       ...
       client.channels.cache.get("893198095528103967").send(`<@${newPresence.userID}> is Online And Uptime Rate - ${uptimerate}% And was Offline for ${hour}h ${minutes}m ${seconds}s`) 
       db.set(`timefr_${newPresence.userID}`, Date.now())
    }
    }
...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    client.channels.cache.get('891125045315895298').send(`Hey <@${newState.member.id}>, Thankyou for being active! You have got some **Disbots Coins** for being active!`)
    }
  }
})
...
client.on('guildMemberRemove', async member => {
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<@${data.ownerID}>'s bot **${bota.username}** has been kicked from server and deleted from website \nReason: Owner left server`)
    });
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<@${serverfind.ownerID}>'s server **${serverfind.name}** has been deleted from website \nReason: Owner left server`)
    });
