...
const dbl = new disbots("5UqN6J6Y84KKK2vYtCrZ5Ol7kdXlNdmBhZ9CdOuHg23RoAiRCdEMP1PZvjjfjwRmUPG5ta7UbXila9s1u4H5AOYpU41IR6MMrGRKZhzE52xSJpYs2HDuFTql6H2Bb4iU", client);
client.on("ready", () => {
    dbl.serverCount(10);
})
...
app.post("/admin/news", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  const Hook = new webhook.Webhook("https://discord.com/api/webhooks/893845667510878210/3vVDv4DKu_FuzWX4_KhQHkGMBKi2qYekBwHfIvpO22n41zb_LfnwVK__2u0qyuv4YI1S");
...
console.log(`
      [===========================================]
              ...
              Re-done by the DisList.Me Team
...
app.use(async (req, res, next) => {
  if (req.path.includes('/admin')) {
    if (req.isAuthenticated()) {
      if (client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.administrator) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.moderator) || req.user.id === "891125044670005260") {
        next();
      } else {
        res.redirect("/error?code=403&message=You is not competent to do this.")
      }
